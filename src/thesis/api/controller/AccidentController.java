...
public class AccidentController extends HttpServlet {
    ...
    private void processs(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        long uid = req.getParameter("uid") != null ? Long.parseLong(req.getParameter("uid")) : 0;

        String content = "";

        pathInfo = pathInfo.toLowerCase();
        CommonModel.prepareHeader(resp, CommonModel.HEADER_JS);
        logger.info("AccidentController.processs: cmd = " + cmd + ", data = " + data);
        ...
        switch (cmd) {
            case "insert_accident":
                content = insertAccident(jsonData);
                break;
            case "get_all_active_accident":
                content = getAllActiveAccident(jsonData);
                break;
            case "get_accident_reported_by_user_id":
                content = getAccidentReportedByUserId(uid, jsonData);
                break;
    ...
    private String insertAccident(JsonObject jsonData) {
        String content = "";
        int ret = -1;
        History history = JsonUtil.fromJson(jsonData, History.class);
        AtomicLong existedHistoryId = new AtomicLong();
        try {
            ...
            switch (ret) {
                case 0:
                    ...
                    ret = MiddleModel.getInstance().checkIsActiveAccident(history, existedHistoryId,AppConst.TYPE_ACCIDENT);
                    if (ret == 0)//accident hiện chưa có trong bảng session
                    {
                        ...
                        if (ret == 0) {
                            ...
                            content = CommonModel.FormatResponse(-1, "insert Accident that bai");
                        }
                        ...
                        if (ret == 0) {
                            ...
                            if (ret == 0) {
                                content = CommonModel.FormatResponse(1, "Tai nạn đã được report trước đó bỏi người khác, thông tin bạn"
                                        + " vừa report đã được merge với vụ tai nạn cũ", existedHistory);
                            }
                        } else {
                            content = CommonModel.FormatResponse(-1, "insert Accident that bai");
                        }
            ...
            logger.info(getClass().getSimpleName() + ".insertAccident: " + ex.getMessage());
            content = CommonModel.FormatResponse(-1, "insert Accident that bai");
    ...
    private String getAllActiveAccident(JsonObject jsonData) {
        String content = "";

        try {
            int ret = -1;
            List<History> listActiveAccident = new ArrayList<>();
            ret = AccidentModel.getInstance().getAllActiveAccident(listActiveAccident);
            if (ret == 0) {

                content = CommonModel.FormatResponse(0, "Lấy danh sách accident thành công", JsonUtil.toJsonTree(listActiveAccident));
            } else {
    ...
    private String getAccidentReportedByUserId(long uid, JsonObject jsonData) {
        String content = "";

        try {
            int ret = -1;
            List<History> listActiveAccident = new ArrayList<>();
            ret = AccidentModel.getInstance().getListAccdientByUserId(uid, listActiveAccident);
            if (ret == 0) {
